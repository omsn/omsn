...
object Admin extends Controller {
		...
		authorize(request,
			request.body.asFormUrlEncoded.map { pairs =>
				...
				if (data.values.exists(_ == None)) {
					...
					Post.findById(id) match {
						case Some(post) => BadRequest(Json.toJson(Map("error" -> ("post with id '" + id + "' already exists"))))
						case None => {
							val categoryId = data.get("categoryId").get.get
							Category.findById(categoryId) match {
								case None => NotFound(Json.toJson(Map("error" -> ("no category with id '" + categoryId + "' found"))))
								case Some(category) => {
	...
	def update(id: String) = Action { implicit request =>
		authorize(request,
			request.body.asFormUrlEncoded.map { pairs =>
				...
				if (Post.update(id, category, pairs.get("title").map(_(0)), pairs.get("html").map(_(0)))) {
					Ok(Json.toJson("success"))
				} else {
	...
	def s3store(mode: String, files: Seq[FilePart[TemporaryFile]]): Seq[String] = s3store(mode, files, Seq())

	def s3store(mode: String, files: Seq[FilePart[TemporaryFile]], paths: Seq[String]): Seq[String] = {
		files match {
			case Nil => paths
			case file :: rest =>
				mode match {
