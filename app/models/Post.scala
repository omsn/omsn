...
object Post {
	...
	val simple = {
		get[Pk[String]]("id") ~
			...
			get[String]("title") ~
			get[String]("html") map {
				case id ~ created ~ categoryId ~ title ~ html => Post(id, created, Category.findById(categoryId).get, title, html)
			}
	}
		...
		DB.withConnection { implicit connection =>
			SQL("select id, created, categoryId, title from post order by created desc").as(
				...
				get[String]("title") map {
					case id ~ created ~ categoryId ~ title => Post(id, created, Category.findById(categoryId).get, title, "")
				} *)
		}
	}
	...
	def create(id: String, category: Category, title: String, html: String): Boolean = {
		DB.withConnection { implicit connection =>
			SQL("insert into post (id, created, categoryId, title, html) values ({id}, {created}, {categoryId}, {title}, {html})")
				.on('id -> id,
